...
public class ConsumerPartitionAssignSeek {
    ...
    public static void main(String[] args) {
        ...
        props.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "group_pizza_assign_seek_v001"); // 특정 파티션에서 특정 오프셋을 읽어올 때는 그룹 아이디를 매번 재설정해줘야 함.
        props.setProperty(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "false"); // default true

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        TopicPartition topicPartition = new TopicPartition(topicName, 0);
//        consumer.subscribe(List.of(topicName));
        consumer.assign(List.of(topicPartition)); // 특정 partition을 assign
        consumer.seek(topicPartition, 5L); // 특정 offset으로 이동
